...
namespace ECommerce.API.ExceptionHandlers
{
    public sealed class StoreExceptionHandler : IExceptionHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            _logger.LogError(
                exception,
                "[Store Module] {ExceptionType}: {Message} | TraceId: {TraceId} | Path: {Path} | TenantId: {TenantId}",
        ...
        {
            return exception is DomainException or Store.Application.Exceptions.ApplicationException;
        }
        ...
        private static (int StatusCode, string Title, string Type) MapException(Exception exception)
        {
            return exception switch
            {
                ...
                Store.Application.Exceptions.ApplicationException => (
                    StatusCodes.Status400BadRequest,
                    "Application Error",
                    "https://tools.ietf.org/html/rfc9110#section-15.5.1"
                ),
        ...
        private string GetDetail(Exception exception)
        {
            if (!_environment.IsDevelopment())
            {
                return exception switch
                {
                    DomainException or Store.Application.Exceptions.ApplicationException => exception.Message,
